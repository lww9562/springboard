...
import org.springboard.board.controllers.admins.BoardSearch;
import org.springboard.board.entities.Board;
import org.springboard.board.entities.QBoard;
...
public class BoardConfigListService {
	...
	public Page<Board> gets(BoardSearch boardSearch) {
		...
		if(sopt != null && !sopt.isBlank() && skey != null && !skey.isBlank()) {
			...
			} else if(sopt.equals("bId")) {		// 게시판 아이디(bId) 검색
				andBuilder.and(board.bId.contains(skey));

			} else if(sopt.equals("bName")) {	// 게시판명(bName) 검색
